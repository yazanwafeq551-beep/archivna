...
| النشر التلقائي | مفعّل: كل دفع إلى `main` ينشر للإنتاج، وإلى `development` ينشر معاينة |
| الخادم وقاعدة البيانات | **لم تُستضف بعد** — الواجهة تعمل وتعرض حالات فارغة حتى تُستضاف |

**الخطوة المتبقية الوحيدة على Vercel** بعد استضافة الخادم: أضف متغيّر البيئة
`VITE_API_URL` بقيمة أصل الخادم (مثل `https://api.arsheefna.com`) في
Project Settings ← Environment Variables، ثم أعد النشر. بدونه تذهب طلبات
الواجهة إلى Vercel نفسه فتعود بصفحة HTML بدل بيانات، ويعرض الموقع محتوى فارغًا.
