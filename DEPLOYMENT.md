...
| قاعدة البيانات | **لم تُنشأ بعد** |
| تخزين الوسائط | **لم يُنشأ بعد** |
...
1. **Neon**: أنشئ مشروعًا، وانسخ **رابط الاتصال المجمَّع (pooled)**.
2. **Cloudinary**: من لوحة التحكم انسخ `Cloud name` و`API Key` و`API Secret`.
