...
| الخادم `apps/api` | **حي** — <https://archivna-api.onrender.com> على Render، منشور من `render.yaml` |
...
يرجّع `{"status":"ok",...}`. النقطة تستعلم قاعدة البيانات فعلًا، فنجاحها يعني أن الخادم والقاعدة موصولان.
...
4. ~~Vercel~~ — تمّ: `VITE_API_URL` مضبوط على `https://archivna-api.onrender.com` لبيئة الإنتاج.

   **معاينات الفروع**: `FRONTEND_URL` على Render يسمح بنطاق الإنتاج فقط، فطلبات المعاينة سيرفضها CORS. أضف نطاق المعاينة إلى `FRONTEND_URL` (يقبل قائمة مفصولة بفواصل) و`VITE_API_URL` لبيئة Preview إن أردت تشغيلها.
